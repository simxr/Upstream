...
func (client *GitHubClient) searchMergedPullRequests(ctx context.Context, repository, username string) ([]GitHubIssue, error) {
	query := fmt.Sprintf(
		"repo:%s is:pr is:merged author:%s",
		repository,
		username,
	)

	var pullRequests []GitHubIssue
	for page := 1; page <= maxSearchPages; page++ {
		parameters := url.Values{}
		parameters.Set("q", query)
		parameters.Set("sort", "updated")
		...
		var result searchResponse
		if err := json.Unmarshal(body, &result); err != nil {
			return nil, fmt.Errorf("decode merged pull request page %d: %w", page, err)
		}
		pullRequests = append(pullRequests, result.Items...)
		if len(result.Items) < 100 || len(pullRequests) >= result.TotalCount {
			break
		}
	}
